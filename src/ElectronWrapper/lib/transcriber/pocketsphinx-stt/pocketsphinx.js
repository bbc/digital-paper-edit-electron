...
//https://github.com/cmusphinx/pocketsphinx/blob/master/doc/pocketsphinx_continuous.1
  ...
  const ps = spawn(Path.join(__dirname, 'pocketsphinx/bin/pocketsphinx_continuous'), args, options);
...
// </s> 2.650 3.040 1.000000
  ...
  ps.on('error', function(error) {
    ...
    console.info("transcript on close:", error);
    // cb(transcript);
    reject(error)
  });
})
